package com.yigu.shop.base;

/**
 * Created by brain on 2016/7/3.
 */
public class RequestCode {
    public static final int login_exit = 0x01;
    public static final int add_addr = 0x02;
    public static final int sel_addr = 0x03;
    public static final int sel_board = 0x04;
    public static final int Task_Detail = 0x05;
    public static final int Danager_Detail = 0x06;
    public static final int assign_license_person = 0x07;
    public static final int sel_otherPerson = 0x08;
    public static final int add_request = 0x09;
    public static final int complete_request = 0x10;
}
